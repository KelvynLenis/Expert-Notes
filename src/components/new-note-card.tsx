...
export function NewNoteCard({ onNoteCreated }: NewNoteCardProps) {
  ...
  function handleStartRecording() {
    ...
    if(!isSpeechRecognitionAPIAvailable) {
      toast.error('Seu navegador não suporta a funcionalidade de gravação de áudio');
      return;
    }
            ...
            <div className='flex flex-1 flex-col gap-3 p-5'>
...

              {
                shouldShowOnBoarding ? (
                  ...
                  <textarea autoFocus className='text-small leading-6 text-slate-400 bg-transparent resize-none flex-1 outline-none' value={content} onChange={handleContentChanged} />
                )
              }
            </div>
